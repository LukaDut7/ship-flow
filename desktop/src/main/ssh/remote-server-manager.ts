...
function exec(client: Client, command: string): Promise<{ stdout: string; stderr: string; code: number }> {
...
export async function checkRemoteNode(client: Client): Promise<string> {
...
export async function checkRemoteServer(client: Client): Promise<string | null> {
...
export async function deployRemoteServer(
  client: Client,
  ...
  await new Promise<void>((resolve, reject) => {
    client.sftp((err, sftp) => {
      ...
      const localTarPath = path.join(app.getPath("temp"), tarName)

      try {
        execSync(`tar -czf "${localTarPath}" -C "${path.dirname(standaloneDir)}" "${path.basename(standaloneDir)}"`)
      } catch (tarErr) {
        sftp.end()
      ...
      writeStream.on("close", () => {
        sftp.end()
        // Clean up local tar
        try { fs.unlinkSync(localTarPath) } catch {}
        resolve()
      })
  ...
  const { code } = await exec(
    client,
    `cd /tmp && tar -xzf shipflow-server.tar.gz && cp -r standalone/* ${REMOTE_DIR}/ && rm -rf standalone shipflow-server.tar.gz`
  )
  if (code !== 0) {
...
export async function reuseIfRunning(client: Client): Promise<number | null> {
  const { stdout: pidStr, code: pidCode } = await exec(client, `cat ${REMOTE_DIR}/${PID_FILE} 2>/dev/null`)
  if (pidCode !== 0 || !pidStr) return null
  ...
  if (killCode !== 0) return null

  // Find the port it's listening on
  const { stdout: portStr } = await exec(
    client,
    `cat ${REMOTE_DIR}/server.port 2>/dev/null`
  )
...
export async function startRemoteServer(client: Client): Promise<{ pid: number; port: number }> {
  // Find a free port on the remote
  const { stdout: portStr } = await exec(
    client,
    `python3 -c "import socket; s=socket.socket(); s.bind(('127.0.0.1',0)); print(s.getsockname()[1]); s.close()" 2>/dev/null || node -e "const s=require('net').createServer();s.listen(0,'127.0.0.1',()=>{console.log(s.address().port);s.close()})"`
  )
  const port = parseInt(portStr, 10)
  if (isNaN(port)) {
  ...
  const { stdout: pidStr, code } = await exec(
    client,
    `cd ${REMOTE_DIR} && ${envVars} nohup node server.js > server.log 2>&1 & echo $!`
  )
  ...
  let ready = false
  for (let i = 0; i < 30; i++) {
    const { code: curlCode } = await exec(
      client,
      `curl -s -o /dev/null -w "%{http_code}" http://127.0.0.1:${port}/api/health 2>/dev/null | grep -q 200`
    )
    if (curlCode === 0) {
      ready = true
      break
...
export async function stopRemoteServer(client: Client, pid: number): Promise<void> {
  await exec(client, `kill ${pid} 2>/dev/null`)
  await exec(client, `rm -f ${REMOTE_DIR}/${PID_FILE} ${REMOTE_DIR}/server.port`)
}

async function stopRemoteServerIfRunning(client: Client): Promise<void> {
